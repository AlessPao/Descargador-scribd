...
class ScribdDownloader {
    ...
    async execute(url, flag) {
        ...
        if (url.match(scribdRegex.DOCUMENT)) {
            await fn(`https://www.scribd.com/embeds/${scribdRegex.DOCUMENT.exec(url)[4]}/content`)
    ...
    async embeds_default(url) {
        ...
        if (m) {
            let id = m[3]  // Ahora el ID está en el índice 3
    ...
    async embeds_image(url) {
        ...
        if (m) {
            ...
            // generate pdf
            const finalPdfPath = `${output}/${sanitize(filename == "title" ? title : id)}.pdf`;
            await pdfGenerator.generate(images, finalPdfPath);
